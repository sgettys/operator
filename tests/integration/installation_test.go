...
var _ = Describe("Installation Lifecycle", func() {
	Context("When an installation is changed", func() {
		It("Should run porter", func() {
			...
			Expect(waitForPorter(ctx, inst, inst.Namespace, inst.Name, "waiting for the bundle to install")).Should(Succeed())
			...
			Expect(waitForPorter(ctx, inst, inst.Namespace, inst.Name, "waiting for the bundle to upgrade")).Should(Succeed())
			validateResourceConditions(inst)

			Log("uninstall the installation")
			inst.Spec.Uninstalled = true
			patchInstallation(inst)
			Expect(waitForPorter(ctx, inst, inst.Namespace, inst.Name, "waiting for the bundle to uninstall")).Should(Succeed())
			validateResourceConditions(inst)
...
// func waitForP(ctx context.Context, inst *porterv1.Installation, msg string) error {
